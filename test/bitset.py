...
#	def set(start, stop, val):
	...
	def __setitem__(self, s, value):
		...
		try:
			...
			remainder = 0
			bitblock = 1			
			if(step == 1 and width >= 4):
				count = width/4	#number of 1111s to use
				remainder = width%4
				bitblock = 15		#1111 base 2
				increment = 4
			else:
				increment = 1
				
			subloop = 1				
			for position in range(start, stop, increment):
				if(count == 0 and increment > 1):
					bitblock = 1
					subloop = remainder
				
				for n in range(0,subloop):
					pos = len(self) - position - 1
					if value: self.value |= (bitblock << pos)
					else: self.value &= ~(bitblock << pos)
					count -= 1
