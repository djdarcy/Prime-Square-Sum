...
from utils.log_lib import (
    OutputManager,
    init_output,
    get_output,
)
from utils.log_lib.channels import (
    ChannelConfig,
    parse_channel_spec,
    format_channel_list,
    KNOWN_CHANNELS,
...
from utils.log_lib.levels import (
...
# =============================================================================
# Vals Channel Integration
# =============================================================================

class TestValsChannel:
    """Test vals channel value capture in match output."""

    def test_format_match_without_vals(self):
        """Match without __vals__ renders clean output."""
        from utils.cli import format_match
        result = format_match({'n': 7})
        assert result == "Found: n=7"
        assert "[" not in result

    def test_format_match_with_vals(self):
        """Match with __vals__ renders annotated output."""
        from utils.cli import format_match
        result = format_match({'n': 7, '__vals__': [666, 666], '__ops__': ['==']})
        assert "Found: n=7" in result
        assert "[666 == 666]" in result

    def test_format_match_vals_json(self):
        """Match with __vals__ in JSON format includes values."""
        import json
        from utils.cli import format_match
        result = format_match(
            {'n': 7, '__vals__': [666, 666], '__ops__': ['==']},
            format_type="json"
        )
        data = json.loads(result)
        assert data["found"] is True
        assert data["variables"] == {"n": 7}
        assert data["values"] == [666, 666]

    def test_format_match_vals_chained(self):
        """Chained comparison vals render correctly."""
        from utils.cli import format_match
        result = format_match({
            'n': 5,
            '__vals__': [1, 5, 10],
